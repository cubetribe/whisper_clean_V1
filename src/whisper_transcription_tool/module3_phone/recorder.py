...
class AudioRecorder:
    ...
    def _recording_worker(self, session: RecordingSession) -> None:
        ...
        try:
            ...
            # Dynamische Anpassung der Kanalanzahl an Gerätekapazitäten
            try:
                # Geräteinfos abfragen
                input_info = sd.query_devices(input_device)
                output_info = sd.query_devices(output_device)
                
                # Sichere Kanaleinstellungen bestimmen
                input_channels = min(input_info['max_input_channels'], 2)  # Max. 2 Kanäle
                output_channels = min(output_info['max_output_channels'], 2)
                
                # Sichere Kanalanzahl wählen (kleinste der drei Werte)
                safe_channels = min(input_channels, output_channels, config.channels)
                
                if safe_channels < 1:
                    raise ValueError(f"Geräte unterstützen keine Kanäle: Input={input_channels}, Output={output_channels}")
                    
                logger.info(f"Angepasste Kanal-Konfiguration: {safe_channels} (original: {config.channels})")
                
                # Audio-Stream mit sicherer Kanalanzahl starten
                with sd.Stream(device=(input_device, output_device),
                               samplerate=config.sample_rate,
                               blocksize=1024,
                               channels=safe_channels,  # Dynamisch angepasste Kanalanzahl
            ...
            # Aufru00e4umen
            logger.info("Aufnahme erfolgreich gestoppt")
            # Session beenden: active_session_id zurücksetzen und Frontend benachrichtigen
    ...
    def stop_recording(self) -> bool:
        ...
        logger.info(f"Stoppe Aufnahme für Sitzung {self.active_session_id}")
        
        # Signal zum Stoppen senden
        self.stop_event.set()
        
        # Auf Thread-Ende warten mit verbesserter Fehlerbehandlung
        if self.recording_thread and self.recording_thread.is_alive():
            try:
                # Längeren Timeout verwenden, um mehr Zeit für die Bereinigung zu geben
                logger.debug("Warte auf Beendigung des Aufnahme-Threads...")
                self.recording_thread.join(timeout=10.0)
                
                # Prüfen, ob der Thread wirklich beendet wurde
                if self.recording_thread.is_alive():
                    logger.warning("Aufnahme-Thread konnte nicht innerhalb des Timeouts beendet werden")
                    # Thread trotzdem als beendet betrachten, um UI nicht zu blockieren
                    self.active_session_id = None
                    self._trigger_callbacks('on_stop', session=session)
                    return True
            except Exception as e:
                logger.error(f"Fehler beim Beenden des Aufnahme-Threads: {e}")
                # Thread als beendet betrachten, um Blockierung zu vermeiden
                self.active_session_id = None
                self._trigger_callbacks('on_error', session=session, error=str(e))
                return False
